-- PostgreSQL Schema for OrganLink
...
WHERE o.availability_status = 'available' 
  ...
  AND o.expiry_time > CURRENT_TIMESTAMP;

-- 3. Function for Matching (PostgreSQL equivalent of stored procedure)
CREATE OR REPLACE FUNCTION perform_priority_matching()
...
BEGIN
    ...
    WHERE expiry_time <= CURRENT_TIMESTAMP AND availability_status = 'available';
    ...
    WHERE o.availability_status = 'available' AND r.status = 'waiting'
      AND p1.blood_group = p2.blood_group AND o.expiry_time > CURRENT_TIMESTAMP
    ...
    IF v_organ_id IS NOT NULL THEN
        ...
        RETURN QUERY SELECT 'SUCCESS'::TEXT AS result;
    ELSE
        RETURN QUERY SELECT 'NO_MATCH'::TEXT AS result;
    END IF;
END;
$$;
